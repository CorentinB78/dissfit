...
from qat.lang import AbstractGate, qrout, X, Y, Z, H, S, RX, RY, RZ, CNOT, CSIGN, PH, SWAP, QRoutine, Program
...
sys.path.append('home/mplazanet/qat/qat-fermion/misc/scripts/dissipative_fit/garnet-benchmarking-data.json')
...
# from qat.fermion.trotterisation import _number_operator_jw, _excitation_operator_jw  
from math import pi

def PRX_routine(theta, phi):
        routine = QRoutine()
...
PRX.set_dag(lambda theta, phi: PRX(-theta, phi))
...
def trotter_lindblad_noise(hamiltonian: ElectronicStructureHamiltonian,
    ...
    def encoding(orig_mode: int, anc_qubit: int, bath_qubit: int, nb_qubits: int = nb_qubits):
        ...
        if compiler is None : 
            ...
            return qr.to_circ().compile(compiler).circuit
    ...
    def encoding_block(block_idx: int):
        ...
        for bath_idx in block_list[block_idx]:
            ...
            circ += encoding(bath_mode, bath_ancilla_map[bath_idx], qubit_mapping[bath_mode])
        return circ
    ...
    def evolve_block(delta_t: float, block_idx: int):
        ...
        barrier_circ = qr.to_circ()
        block_circ = encoding_block(block_idx)
        block_circ.gate_set.add_signature(PRX)
        dagger_from_params = {
            "PRX": lambda params: [- params[0], params[1]],
                }
        block_circ_dag = block_circ.dag(dagger_from_params)
        circ += block_circ_dag
        ...
        circ2 = encoding_block(block_idx)
        circ += circ2
        if compiler is None : 
            circ += qr.to_circ()
        else : 
            circ += qr.to_circ().compile(compiler).circuit
        
        #print(circ + circ2)
        #circ += encoding_block(block_idx),
        return circ, barrier_circ
    ...
    def artificial_noise(delta_t):
        ...
        if compiler is None : 
            ...
            return qr.to_circ().compile(compiler)
    ...
    def trotter_step_first_order(delta_t):
        """prog = Program()
        _ = prog.qalloc(nb_qubits)
        circ = prog.to_circ()"""
        circ_imp = evolve_impurity(delta_t)
        for block_idx in range(len(block_list)):
            trotter_circ, barrier_circ = evolve_block(delta_t, block_idx)
            prog = Program()
            qubits = prog.qalloc(nb_qubits)
            if block_idx < len(block_list) - 1:  # if not the last one
                prog.apply(CSIGN, [imp_qubits[0], block_ancilla[block_idx]])
                trotter_circ += prog.to_circ()
        prog = Program()
        qubits = prog.qalloc(nb_qubits)
        ...
        trotter_circ += prog.to_circ()
        if compiler is None : 
            trotter_circ_compiled = trotter_circ
        else : 
            trotter_circ_compiled = trotter_circ.compile(compiler).circuit
        return circ_imp + barrier_circ + trotter_circ_compiled
    ...
    def full_evol(time: float) -> QRoutine:
        ...
        for _ in range(n_trotter_steps):
            trotter_step_circ = trotter_step(dt)
            #trotter_step_circ.display()
            #print('number of trotter steps:', n_trotter_steps)
            """print("number of gates per trotter step:")
            print("PRX: ",  trotter_step_circ.count('PRX'))
            print("CZ: ",  trotter_step_circ.count('CSIGN'))
            print("depth: ",  trotter_step_circ.depth(default = 1))"""
            circ += trotter_step_circ
            if with_noise_gates:
                circ += artificial_noise(dt)
            if with_wait_gates:
                circ += wait()
        ...
        if with_outer_encoding:
            ...
            qubits = prog.qalloc(nb_qubits)
            if with_barriers:
                prog.apply(barrier[nb_qubits](), qubits)
            circ += prog.to_circ()
            circ += encoding_all().dag()
        return circ
...
def noise_as_a_resource_simulation(hamiltonian: ElectronicStructureHamiltonian, 
    ...
    """hm, _ =get_Garnet_hwmodel(qubits =[19, 10, 14, 15, 16, 9, 11, 4, 5])
    hm.gates_specification.gate_times = gate_times
    print(gate_times)"""
    hm = HardwareModel(GatesSpecification(gate_times, {}))
    evol_circ, encoding, qubit_mapping = trotter_lindblad_noise(hamiltonian, 
                                                              ...
                                                              compiler = compiler)
    evol_5 = evol_circ(5*trotter_time_step)
    phys_time_5 = total_duration(evol_5, hm)
    phys_time_10 = total_duration(evol_circ(10 * trotter_time_step), hm)

    #phys_time_5 = evol_5.depth(gate_times = gate_times)
    #phys_time_10 = evol_circ(10*trotter_time_step).depth(gate_times = gate_times)
    ...
    return evol_circ, encoding, qubit_mapping, gate_times, trotter_step_phys_time
